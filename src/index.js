...
async function startBot() {
  try {
    ...
    // ВАЖНО: Пользовательские обработчики регистрируем ПЕРВЫМИ, чтобы команды /start, /catalog, /cabinet
    // обрабатывались до того, как админский bot.on('text') может их перехватить
    console.log('[START] Шаг 5: Настройка обработчиков...');
    console.log('[START] Настройка пользовательских обработчиков (ПЕРВЫМИ, чтобы команды обрабатывались)...');
