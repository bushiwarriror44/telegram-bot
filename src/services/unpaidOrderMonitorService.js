...
export class UnpaidOrderMonitorService {
    constructor(bot) {
        ...
        this.processedOrders = new Set(); // Для отслеживания уже обработанных заказов
    }
    ...
    async checkUnpaidOrders() {
        try {
            ...
            for (const user of allUsers) {
                try {
                    ...
                    if (unpaidOrders.length > 0) {
                        // Проверяем, не обрабатывали ли мы уже эти заказы
                        const newOrders = unpaidOrders.filter(
                            order => !this.processedOrders.has(`${chatId}_${order.id}`)
                        );
                        
                        if (newOrders.length > 0) {
                            // Берем последний неоплаченный заказ
                            const lastUnpaidOrder = newOrders[0];
                            
                            // Помечаем заказ как обработанный
                            this.processedOrders.add(`${chatId}_${lastUnpaidOrder.id}`);
            ...
            // Очищаем старые записи из processedOrders (старше 1 часа)
            // Это нужно, чтобы не накапливать память
            if (this.processedOrders.size > 10000) {
                this.processedOrders.clear();
                console.log('[UnpaidOrderMonitor] Очищена история обработанных заказов');
            }
        } catch (error) {
